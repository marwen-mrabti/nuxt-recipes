...
<template>
  <main class="container mx-auto space-y-6 py-4 px-4">
    ...
    <UButton color="secondary" class="w-fit px-2" @click="() => refresh()">
      refetch
    </UButton>
    ...
    <ul v-else class="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4">
      <li v-for="recipe in recipes" :key="recipe.id" class="aspect-square grid grid-rows-subgrid row-span-3 gap-4 border-2 border-neutral-200 rounded-sm p-2">
        ...
        <p class="text-pretty flex items-center w-full gap-2">
          <span>rating: </span>
          <span v-for="(_, index) in Math.floor(recipe.rating)" :key="index" class="flex flex-row gap-1 items-center">
            <UIcon name="i-tabler-star-filled" class="size-5 text-amber-400" />
          </span>
        </p>
      </li>
    </ul>
